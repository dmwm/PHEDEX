...
PHEDEX.Core.Widget.DataTable = function(divid, opts) {
    ...
    //******************************************************************************************************************************
    this.buildTable = function(div, columns, map, dsschema) {
    ...
    //*******************************************************************************************************
    //Function:removeBtnMenuItem
    //Purpose :This function removes the given menu item from the split menu button
    //*******************************************************************************************************
    this.removeBtnMenuItem = function(name) {
        var m = this.column_menu.getItems();
        for (var i = 0; i < m.length; i++) {
            if (name == m[i].value) {
                this.column_menu.removeItem(m[i].index);
                this.refreshButton();
                return;
            }
        }
