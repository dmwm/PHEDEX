...
//     { name: 'phedex-navigator', requires: ['phedex-core-widget','phedex-widget-nodes'] },
    ...
    { name: 'phedex-module',      requires:['phedex-core-app'] },
  ];
  ...
  var _callback = function(args) {
    ...
    item = args[2];
    switch (type) {
      case 'Progress': { myLog(ev+': '+type+', '+item.name); break; }
      case 'Success':  {
	var l='';
	...
	break;
      ...
      case 'Failure':  { myLog(ev+': '+type+', '+item.name); break; }
      case 'Timeout':  { myLog(ev+': '+type); break; }
    };
  };
