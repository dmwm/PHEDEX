...
// This is for dynamically loading data.
  that.loadNodeData=function(node, fnLoadComplete) {
// First, a callback function, that identifies what to do with the returned data.
    var loadNodeData_callback = function(result) {
//    If the load-call failed, simply close-out the operation
      if ( typeof(result.link[0]) == 'undefined' ) { fnLoadComplete(); return; }

//    Build child tree-nodes to insert into the node we clicked on.
//   This should depend on what level of the tree we clicked on, different levels are generated by different widgets
      if ( result.request_call == 'TransferQueueBlocks' )
      {
	    ...
	    tNode1.payload = { Call: 'TransferQueueFiles', obj: node.payload.obj, args: {} }; // so I can use this in the callback
...
//  Now, we trigger a call to the data-service to get the right type of data. Again, depends on the level we are at.
//  If I don't know where I am, close out the operation
    if ( typeof(node.payload) == 'undefined' ) { fnLoadComplete(); return; }

//  From the top-level (nodes), the next level is achieved with a 'TransferQueueBlocks' call
    if ( node.payload.Call == 'TransferQueueBlocks' )
    {
      PHEDEX.Datasvc.TransferQueueBlocks(node.payload.args,node.payload.obj,loadNodeData_callback);
    }
    else if ( node.payload.Call == 'TransferQueueFiles' )
    {
      PHEDEX.Datasvc.TransferQueueFiles(node.payload.args,node.payload.obj,loadNodeData_callback);
    }
    else { fnLoadComplete(); }
  }
...
//    Hack? Adding a 'payload' object allows me to specify what PhEDEx-y thing to call to get to the next level
      tNode.payload = { Call: 'TransferQueueBlocks', obj: this , args: {} }; // so I can use this in the callback
...
// PHEDEX.Widget.LinkNode=function(site,direction,parent,div,data_queue,data_hist,data_error) {
//   var that = new PHEDEX.Core.Child.Widget(div,parent,null);
//   that.site=site;
//   that.direction=direction;
//   if (that.direction=='to') that.antidirection='from';
//   else that.antidirection='to';
