...
PHEDEX::Web::API::Shift::Requested - Requested data summary

=head1 DESCRIPTION

Returns information about the data requested for each node in hourly 
    ...
    foreach $i ( keys %{$r} )
    {
      if ( $i =~ m%^T1_(.*)_Buffer$% )
      {
        ...
        foreach $bin ( keys %{$r->{$i}} )
        {
          ...
          {
            $r->{$node}{$bin} = $r->{$i}{$bin};
            $r->{$node}{$bin}{REQUEST_BYTES} = 0;
          }
          $r->{$node}{$bin}{REQUEST_BYTES} += $r->{$i}{$bin}{REQUEST_BYTES};
        }
        delete $r->{$i};
      }
    }
