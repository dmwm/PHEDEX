...
# for cases where entire sql scripts are generated by a script, this function
# allows them to be 'printed' directly to SQL*Plus. This is better than
# forking SQL*Plus with the connection string on the command-line, because
# that exposes the connection string to process monitoring tools
#
# The function returns a filehandle, just print your script to it and close
# it when you're done.
sub sqlPlus {
  my $self = shift;
  open FH, "| sqlplus -L -S" or die "open: sqlplus: $!\n";
  print FH $self->{DBH_DBUSER},'/',$self->{DBH_DBPASS},'@',$self->{DBH_DBNAME},"\n";
  return *FH;
}
