package PHEDEX::Custom::SRM::Stager::Agent;
use strict;
use warnings;
...
sub getStagerFiles
{
    ...
    #
    # Since srm-meta-data seems to hang every once in a while, blocking
	...
	my @args = map { "-M $$_{PFN}" } @slice;
        open (QRY, "$pfx srm-get-metadata -retry_num=1 @args |")
            or do { $self->Alert ("srm-get-metadata: cannot execute: $!"); return undef };
        ...
        while (<QRY>)
        {
            ...
            next if ! /\W+(\w+)\s:(\S+)$/;
            do { $self->Alert ("srm-get-metadata output unrecognised for file: $pfn"); next }
                if ! exists $pfn2f{$pfn};
            $pfn2f{$pfn}{$1} = $2;
...
sub idle
{
	...
	while (@requests)
	{
	    ...
	    $self->Alert ("srmstage failed: @{[&runerror($rc)]}") if ($rc);
