package PHEDEX::Custom::SRM::Migrate::Agent;
use strict;
use warnings;
...
sub new
{
    ...
    my %params = (DBCONFIG => undef,		# Database configuration file
		  ...
		  PROTOCOLS => [ 'srm' ],	# Protocols to accept
...
sub idle
{
	...
	while (my ($task, $dest, $size, $lfn, $pfn, $available) = $q->fetchrow())
	{
		...
		or do { $self->Warn ("no metadata for $pfn: $!"); next };
            while(<SRM>) {
                if (/isPermanent :true/) {
                    $migrated = 1;
                }
	    ...
	    close (SRM)
		or do { $self->Warn ("no migration info for $pfn: $?"); next };
	    next if !$migrated;
