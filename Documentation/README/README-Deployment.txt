...
*** Install the software
  ...
  wget -O $sw/aptinstaller.sh \
    http://cmsdoc.cern.ch/cms/cpt/Software/download/lt3/aptinstaller.sh
  chmod +x $sw/aptinstaller.sh
  
  $sw/aptinstaller.sh -path $sw setup
  eval `$sw/aptinstaller.sh -path $sw config -sh`
  apt-get update
  apt-get install cms+PHEDEX+PHEDEX_2_3_2
...
as will be explained below.  If you used the commands above, use:
  source $sw/slc3_ia32_gcc323/cms/PHEDEX/PHEDEX_2_3_2/etc/profile.d/env.sh
