...
Good examples available:
  PHEDEX/Custom/Template/FileDownloadVerify
...
Good examples available:
  PHEDEX/Custom/Template/FileDownloadDelete
...
** How to check log files locally and debug transfers

1) The PhEDEx download agent keeps a history of all the output
   generated by the transfer processes. Those informations are
   available in a history sub-dir located in the download agents state
   directory. Typically this is:
   <PHEDEX_BASE>/<InstanceName_NodeName>/state/<AgentName>/archive

2) In order to acces the log file of a specific download trial, you
   need to get the
