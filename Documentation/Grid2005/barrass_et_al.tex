...
Historically High Energy Physics (HEP) experiments have been able to rely on manpower-intensive mechanisms for distributing data to geographically dispersed collaborators. The new era of CERN Large Hadron Collider (LHC) \cite{lcg:website} computing necessitates a move to more scalable data distribution and management systems; many experiments have identified Grid computing as a suitable basis for creating such systems. In the near future these systems must bridge the gap between ``traditional" HEP data distribution- large scale scheduled transfers between major sites- and more dynamic data distribution, which enables optimized replication in response to user demand.

In general existing HEP distribution networks are very structured, often characterised by simple tree-like toplogies in which nodes are persistent, but routes are typically not in constant saturating use. Access to and production of data is undertaken at large regional centres or computing centres at Institutes or Universities. This level of activity will not scale to the LHC era however; a solution is to utilise resources that are presented to scientific collaborations for opportunisitc use. However, as these resources are typically transient the previously quite stable HEP infrastructure will be afflicted by greater churn. Data distribution in this type of environment is more suited to recent Grid \cite{foster:gridbook} or peer-to-peer filesharing technologies, which are characterised by random topologies to avoid network segmentation.

The HEP distribution environment is therefore a blend of static core infrastructure with an associated infrastructure of transiently available resources. Over the former infrastructure transfers are continuous, sourced from a central site through successive ``tiers" of sites, hosting resources of decreasing capacity (fig. \ref{fig:tiers}) \cite{monarc:paper}). LHC experiments will use this core infrastructure to store multiple remote, secure tape copies of invaluable experimental data in real time, relying on the core infrastructure's more robust and available services. To operate effectively however they will also need to be able to make opportunistic use of transient resources to enable on-demand analysis of new or controversial data.
...
\bigskip
\subsection{CMS and PhEDEx}CMS \cite{cms:tdr}- one of the four LHC experiments currently being built at CERN- has among the most demanding requirements of data management \ref{cms:computingtdr}. It is predicted that in a given year it will need to transfer around 5 PB of data over the WAN, just in order to distribute its raw detector data. Additionally, a further 5 PB of processed data, and a similar quantity of  simulated raw and processed data must also be transferred. This will require the distributed management of millions of file replicas, assuming a filesize of order 10 GB.

CMS has made important progress toward using a functioning datagrid by developing a robust data management layer. This data management layer has a simple twofold goal: to manage the prioritized transfer of files from multiple sources to multiple sinks; and to provide answers to questions of transfer latency and speed to enable scheduling of transfer operations. 
...
Smaller persistent sites that are part of the core experiment infrastructure associate with the regional centres, just as the regional centres associate with the experimental centre. By associating sites in this way the load- in terms of resource access and network bandwidth- on the detector facility is reduced. These smaller sites take part in more dynamic tasks, like the production of detector simulation data, or analysis of data already processed at the detector facility.

The LHC computing era brings access to a dynamic infrastructure of sites that become part of a more dynamic Grid of resources. These Grid sites typically make resources available on something close to a best-effort basis- the extreme example being a physicist’s laptop. These resources are intended for rapidly changing analysis tasks, typically with much fewer resources (storage and bandwidth). This dynamic environment is more ideally suited to management by a grid that optimizes the storage and availability of data, and is driven by end-user analysis tasks.
...
\mycaption{PhEDEx' layered architecture. Each layer is considered more reliable than the one below. }
\end{figure}
...
\mycaption{A sample exchange between agents to manage the replication of a file. Please refer to text for description }
\label{fig:exchange}
\end{figure}
...
The scale of data distribution for the LHC experiments is much larger than previous HEP experiments, and therefore requires distribution management mechanisms that scale to function efficiently in such demanding environments. In areas outside HEP there have been many advances in replication technology in recent years. Data Grids and peer to peer filesharing technologies promise to meet these demands.

\bigskip
\subsection{Previous methods for distributing HEP data}
Existing HEP experiments tend to manage data distribution in structured but manpower intensive ways. One such example is the use of rsync to manage the periodic transfer of data files from an authoratative source to a remote site. This was typically done by issuing commands whenever a transfer was required, rather than by having an automated process manage the transfers. This was the approach taken by, for example, the BaBar experiment \cite{babar:website}. More recently the BaBar experiment has investigated the use of the Storage Resource Broker to distribute and manage access to it's data \cite{babar:srb}; it also has a home grown application named BdbServer++ that enables users to locate distributed replicas and create a complete dataset replica on the user's laptop \cite{babar:distribution}.
...
Most Grid technologies are focused on the running of jobs rather than on the large scale replication of data. For example, Condor \cite{condor:website} and its Grid-equivalent, Condor-G, focus very much on the running of jobs, and views the management of data to be something to be solved by external processes. Similarly, commercial products and services like the Sun Grid Engine \cite{sungrid:website} focus on providing the resources to run jobs rather than managing the replication and custody of data over a number of distributed sites.
...
Grid data management tools, like SRB \cite{srb:website}, focus on providing distributed access to data, and enable users to create replicas within the system. SRB also provides sophisticated functionality, maintaining a sense of master or authoritative files, and ensuring for example that the last replica of a file in the system cannot be accidentally deleted. However, again the system does not provide a mechansim for harvesting files as they are created, and then automatically propogating those files through some structured network.

\bigskip
\subsection{Peer-to-Peer Filesharing}
Peer-to-peer filesharing applications vary between more-centralized systems in which peers and replicas of data are tracked in a manner similar to instant messaging systems, and less centralized in which client communication is more truly peer-to-peer and doesn't rely on centralized services to make contact with peers or find replicas. An example of the former is the original BitTorrent \cite{bittorrent:website}, and of the latter, GnuTella \cite{gnutella:website}.

There are differences between the environment managed by existing peer-to-peer applications and the HEP environment. Typically peer-to-peer systems assume a completely transient, dynamic environment in which most routes are short-lived. To avoid network segmentation in this environment existing peer-to-peer apps create random peer topologies to minimize the consequences of peers losing contact. In contrast, the HEP environment is characterised by well-funded and managed dedicated high-speed links between large international institutions that are relatively long lived.

Furthermore, existing peer-to-peer systems have no sense of the value of data, or any sense of custody of data at sites remote to the original source. Peer-to-peer clients are typically characterised by on-demand transactions rather than the continuous sustained replications that characterise the bulk of HEP distribution.

On the other hand, peer-to-peer filesharing applications do have many characteristics that make them suitable for use in the HEP environment. Decentralisation of system state information makes some systems very robust in the face of resource outages. Also, many peer-to-peer filesharing applications enable the download a file from multiple sources: the file is split into blocks, and the client replicates blocks from whichever peers are capable of serving them. This concept is easy to map onto HEP data without developing mechanisms for splitting files, as HEP data is typically organised into datasets which form the basic unit of access; therefore the basic unit of distribution in HEP is the dataset rather than the file.
