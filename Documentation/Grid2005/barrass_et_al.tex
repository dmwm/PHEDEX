...
In general existing HEP distribution networks are very structured, often characterised by simple tree-like toplogies in which nodes are persistent, but routes are typically not in constant saturating use. Access to and production of data is undertaken at large regional centres or computing centres at Institutes or Universities. This level of activity will not scale to the LHC era however; a solution is to utilise resources that are presented to scientific collaborations for opportunisitc use. However, as these resources are typically transient the previously quite stable HEP infrastructure will be afflicted by greater churn. Data distribution in this type of environment is more suited to recent Grid or peer-to-peer filesharing technologies, which are characterised by random topologies to avoid network segmentation.
