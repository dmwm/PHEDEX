...
file's GUID was then added as additional SRB metadata (SmodD). Tier 1 sites then
transferred the files to their import buffers and mass storage
...
the LRC at CERN periodically. However, as difficulties with the MCat (discussed below)
...
The poor performance of SRB was unexpected. Prior to DC04 it had been used 
successfully by CMS production for data management throughout the pre-production.
For this reason SRB was seen as a valid component of the DC04 system, especially as 
...
disruption to the MCat service crippled the whole of the SRB v2 chain.
...
before DC04. Pre-DC04 planning indicated that files would be of the order of 1GB in size, and
...
During DC04 file sizes were orders of magnitude smaller than
expected. Had this small filesize been apparent before the data
challenge a different strategy for placing files in SRB would have been developed: in retrospect
an export buffer agent based on drop boxes (as used at the Tier 0)
would be a preferred system, separating transfers from the GUID
metadata insertion.  

Since the data challenge came to an end a series
of tests have been undertaken in an attempt to replicate and solve the
problems encountered. This is seen as important as many components of the CMS grid will
rely on similar backends (eg Oracle databases). It is therefore essential to
understand the problems experienced.

Use of the SRB CLI also posed some subtle problems: return codes were not 
reliable, at times returning 0 despite failure; some killed transfers continued to 
run in the background; and the delete command (Srm) was particularily slow and 
caused problems for sites with small disk servers. Many of the problems experienced can be attributed to using the CLI in
...
Mbps over a whole day. These transfer rates
are due to small file sizes, and thus compare favourably with other
chains (the SRB chain was unable to take part in the large filesize tests
at the end of DC04). Prior to the data challenge testing indicated that transfer
rates could be sustained at a rate greater than this.

Recent versions of SRB have included new features developed in response to
CMS experience, for which we would like to thank the developers at SDSC.
