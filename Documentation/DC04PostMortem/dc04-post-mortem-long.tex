...
Querying information from RLS was in general very slow.
Looking up file information by GUID seems sufficiently fast, but bulk queries by GUID took seconds per file. Queries on metadata were too slow- for instance, querying for all files of a specific "owner" took hours. In contrast, it took two minutes to dump the entire catalogue to file after gaining access to the Oracle backend.
